...
function isListSurfaceWithoutSingleBooking(): boolean {
  // Pages that show many reservation numbers — do not pick the first for the detail chip.
  return Boolean(
...
function scrapeStayDates(): { from: string; to: string } {
  ...
  const from = (arrivalRaw && parseEmmaDate(arrivalRaw)) || localTodayIso();
  const to = (departureRaw && parseEmmaDate(departureRaw)) || localTomorrowIso();
  return { from, to };
}
