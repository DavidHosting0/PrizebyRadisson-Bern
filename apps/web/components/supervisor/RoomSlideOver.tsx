...
export function RoomSlideOver({
  ...
  const parseApiError = (raw: string): string => {
    ...
    return raw || tReception('roomDetail.requestFailed');
  };
  ...
  const setStatusMut = useMutation({
    ...
    onSuccess: (next) => {
      ...
      toast.push(
        tReception('roomDetail.toastStatusChanged', {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between border-b border-sidebar-border/60 px-5 py-4">
          <div className="min-w-0">
            <h2 className="truncate text-xl font-semibold tracking-tight text-white">
              {tReception('roomDetail.roomTitle', { roomNumber: data?.roomNumber ?? '…' })}
              {data?.floor != null && (
                <span className="ml-2 text-sm font-normal text-sidebar-muted">
                  {tReception('roomDetail.floorSuffix', { floor: data.floor })}
                </span>
              )}
            </h2>
            {data && (
        ...
        <div className="sidebar-scroll flex-1 overflow-y-auto p-5">
          {isLoading && <p className="text-sm text-sidebar-muted">{tReception('roomDetail.loading')}</p>}
          {data && (
            <div className="space-y-5">
              {canSetStatus && !data.outOfOrder && (
                <section className={APP_DARK_CARD + ' p-4'}>
                  <h3 className="text-xs font-semibold uppercase tracking-wider text-sidebar-muted">
                    {tReception('roomDetail.statusHeading')}
                  </h3>
                  <p className="mt-1 text-xs text-sidebar-muted">
                    {tReception('roomDetail.statusHint')}
                  </p>
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {STATUS_OPTIONS.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          {tReception(`roomDetail.${opt.labelKey}`)}
                        </button>
                      );
                    })}
                  </div>
              ...
              <section className={APP_DARK_CARD + ' p-4'}>
                ...
                <textarea
                  ...
                  placeholder={tSup('notesPlaceholder')}
                />
                <Button
                  variant="secondary"
              ...
              <section className={APP_DARK_CARD + ' p-4'}>
                ...
                <label className="mt-3 flex cursor-pointer items-center gap-3">
                  ...
                  />
                  <span className="text-sm text-slate-100">{tSup('markOutOfOrder')}</span>
                </label>
                <label className="mt-3 block text-xs font-medium text-sidebar-muted">
                  ...
                  <input
                    ...
                    placeholder={tSup('oooReasonPlaceholder')}
                  />
                </label>
                <label className="mt-3 block text-xs font-medium text-sidebar-muted">
                  {tSup('expectedBackInService')}
                  <div className="mt-1">
                    <DateInput
                      type="datetime-local"
                      value={oooUntilLocal}
                ...
                >
                  {tSup('saveMaintenance')}
                </Button>
              </section>

              <section className={APP_DARK_CARD + ' p-4'}>
                <h3 className="text-xs font-semibold uppercase tracking-wider text-sidebar-muted">
                  {tSup('cleaningHeading')}
                </h3>
                <p className="mt-2 text-sm text-sidebar-muted">
                  {data.cleaningDeclaredAt
                    ? tSup('cleaningMarkedClean')
                    : tSup('cleaningNoChecklist')}
                </p>
                <Button
                  variant="danger"
