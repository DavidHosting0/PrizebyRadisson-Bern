...
function FolioChargeTable({ charges }: { charges: ReservationFolioCharge[] }) {
  if (charges.length === 0) {
...
function EmmaFolioSections({ emmaFolio }: { emmaFolio: NonNullable<ReservationDetail['emmaFolio']> }) {
            ...
            <>
              ...
              <Section title="System">
                ...
                {data.folioFetchedAt && (
                  <Field
                    label="Folio geladen"
                    value={new Date(data.folioFetchedAt).toLocaleString('de-CH')}
                  />
                )}
              </Section>

              {data.emmaDetail && <EmmaDetailSections emmaDetail={data.emmaDetail} />}
              {data.emmaFolio && <EmmaFolioSections emmaFolio={data.emmaFolio} />}
            </>
          )}
        </div>
      </aside>
